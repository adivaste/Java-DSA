...
def make_api_request(text):
    ...
    # Include the proxy settings here
    response = requests.post(api_url, headers=headers, json=data, stream=True, proxies=proxy)
...
class QueryHandler(FileSystemEventHandler):
    def on_created(self, event):
        ...
        with open(event.src_path, "r") as query_file:
            ...
            query_text = "(This is an aptitude exam question. Please read the question carefully and answer properly. For code, prefer Java.) " + query_text
            make_api_request(query_text)
